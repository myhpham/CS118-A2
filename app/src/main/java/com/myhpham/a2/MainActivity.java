package com.myhpham.a2;

import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;

public class MainActivity extends AppCompatActivity {

    Button dlImgButton, viewImgButton, searchImgButton;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);

        dlImgButton = findViewById(R.id.dlImagesButton);
        viewImgButton = findViewById(R.id.viewImagesButton);
        searchImgButton = findViewById(R.id.searchImagesButton);

        //download image listener
        dlImgButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(MainActivity.this, DownloadImageActivity.class));
            }
        });

        //view image listener
        viewImgButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(MainActivity.this, ViewImageActivity.class));
            }
        });

        //search image listener
        searchImgButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(MainActivity.this, SearchImageActivity.class));
            }
        });
    }
}
